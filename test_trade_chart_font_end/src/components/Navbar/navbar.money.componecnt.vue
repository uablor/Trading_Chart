...
const selectedIndex = ref(currentMode.value === 'real' ? 0 : 1);
const dropdownActive = ref(false);

const selectedLabel = ref(accounts[selectedIndex.value].value);

const selectedIcon = ref(accounts[selectedIndex.value].icon);
const selectedtypeWallet = ref(accounts[selectedIndex.value].type);
...
const selectAccount = (index) => {
  walletStore.setSelectedWalletMode(accounts[index].type === 'บัญชีจริง' ? 'real' : 'demo');
...
const connectWebSocket = () => {
  ...
  socket.value.onmessage = (event) => {
    ...
    selectedLabel.value = accounts[selectedIndex.value].value;
    selectedtypeWallet.value = accounts[selectedIndex.value].type;
  };
